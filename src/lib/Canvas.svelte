<script lang="ts">
    ...
    class Particle {
        ...
        update(h: number): void {
            ...
            if (distSquared > 0.1 || distSquared < 0.0005) {
        ...
        draw(
            ...
            for (let i = 0; i < pathLength - 1; i++) {
                ...
                const alpha: number = Math.pow(mod(index1 - this.currentIndex, pathLength) / pathLength, 2);
                ...
                ctx.strokeStyle = `hsla(${hue}, ${Math.floor(saturation)}%, ${Math.floor(light)}%, ${alpha})`;
    ...
    $effect(() => {
        ...
        const particles: Particle[] = Array.from({ length: particleAmount }, () => {
            return new Particle(
                system,
                x0 + sampleUniform(-10, 10),
                y0 + sampleUniform(-10, 10),
                z0 + sampleUniform(-10, 10),
                pathLength
            );
        });
        ...
        function draw(timestamp: DOMHighResTimeStamp): void {
            ...
            if (dt <= 0.1) {
                ...
                for (const particle of particles) {
                    particle.update(h);
                    particle.draw(ctx, centerX, centerY, scaleX, scaleY, huePosition, hueRange, hueRotation, saturation, light);
                }
            }
